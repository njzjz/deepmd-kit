...
void merge_neighbor_list(deepmd::InputNlist &nlist,
                         std::vector<deepmd::InputNlist> &nlist_list) {
  int inum = 0;
  for (deepmd::InputNlist nlist_i : nlist_list) {
    inum += nlist_i.inum;
  }
  ...
  nlist = deepmd::InputNlist(inum, ilist, numneigh, firstneigh);
}
