...
template <>
inline void _DP_DeepPotModelDeviCompute<double>(
    DP_DeepPotModelDevi *dp,
    const int natom,
    const double *coord,
    const int *atype,
    ...
    double *atomic_virial)
{
    DP_DeepPotModelDeviCompute(dp, natom, coord, atype, cell, energy, force, virial, atomic_energy, atomic_virial);
}

template <>
inline void _DP_DeepPotModelDeviCompute<float>(
    DP_DeepPotModelDevi *dp,
    const int natom,
    const float *coord,
    const int *atype,
    ...
    float *atomic_virial)
{
    DP_DeepPotModelDeviComputef(dp, natom, coord, atype, cell, energy, force, virial, atomic_energy, atomic_virial);
}
        ...
        public:
            ...
            void init(const std::vector<std::string> &models)
            {
                ...
                for(size_t i = 0; i < models.size(); ++i)
                    cstrings.push_back(const_cast<char*>(models[i].c_str()));

                dp = DP_NewDeepPotModelDevi(&cstrings[0], cstrings.size());
                         ...
                         const std::vector<VALUETYPE> &box)
            {
                ...
                _DP_DeepPotModelDeviCompute_variant<VALUETYPE>(dp, natoms, coord_, atype_, box_, ener_, force_, virial_, nullptr, nullptr);
                         ...
                         const std::vector<VALUETYPE> &box)
            {
                ...
                _DP_DeepPotModelDeviCompute<VALUETYPE>(dp, natoms, coord_, atype_, box_, ener_, force_, virial_, atomic_ener_, atomic_virial_);
